...
echo " "
gron "https://otx.alienvault.com/api/v1/indicators/domain/$target/url_list?limit=100&page=1" | grep "\burl\b" | gron --ungron | jq | grep '\"url\"' | awk -F "\"" '{print $4}' | tee -a allurl.txt 
echo " "
echo -e "${DO}===============================||||||||||==================================${STOP}"
done
